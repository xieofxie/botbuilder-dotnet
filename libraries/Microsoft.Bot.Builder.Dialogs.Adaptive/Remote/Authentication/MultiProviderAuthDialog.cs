    ...
    public class MultiProviderAuthDialog : ComponentDialog
    {
        ...
        private async Task<DialogTurnResult> FirstStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrEmpty(stepContext.Context.Activity.ChannelId) && stepContext.Context.Activity.ChannelId == "directlinespeech")
            {
                ...
                var noLinkedAccountResponse = new Activity()
                {
                    Type = ActivityTypes.Message,
                    Text = string.Format("A token could not be retrieved for {0}, please link your account to your Assistant and try again", connectionName)
                };
                ...
                if (matchingProviders.Count > 1)
                {
                    ...
                    return await stepContext.PromptAsync(
                        ...
                        new PromptOptions
                        {
                            ...
                            {
                                Type = ActivityTypes.Message,
                                Text = "You have multiple accounts configured. Which one would you like to use?"
                            },
                            Choices = choices,
                        },
                        cancellationToken).ConfigureAwait(false);
